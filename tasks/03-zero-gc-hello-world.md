...
The current Windows x64 ZeroGC deliberately configures empty heap and ephemeral
ranges and supplies only a non-null sentinel card table. Empty heap bounds make
checked and bulk-copy paths skip card marking; normal managed references are
below `ephemeral_low`, so the unchecked pre-grow barrier also exits before
indexing the card table. Empty ephemeral bounds alone do not protect bulk copies.
This is valid only while no collection consumes card metadata and the pinned
runtime keeps using the expected barrier variant. The synthetic empty heap
bounds, empty ephemeral range, and sentinel card table must
